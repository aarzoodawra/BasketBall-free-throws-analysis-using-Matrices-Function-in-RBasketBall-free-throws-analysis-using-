...
#Basket Ball free throws Analysis 
...
# Creating Matrix for above vectors

FreeThrows <- rbind(KobeBryant_FT ,JoeJohnson_FT ,LeBronJames_FT ,CarmeloAnthony_FT ,DwightHoward_FT ,ChrisBosh_FT ,ChrisPaul_FT ,KevinDurant_FT ,DerrickRose_FT ,DwayneWade_FT);
FreeThrows

# Now we have matrix we can remove vectors
...
# Checking colnames and rownames
colnames(FreeThrows)

#There are no colnames, Lets assign rownames and colnames.

colnames(FreeThrows) <- Seasons
rownames(FreeThrows) <- Players

FreeThrows # Checking Matrix now
...
# We can see from the graph tgar Chris paul has the highest accuracy in free throws but he gets very less number of free throws attempts
# And also we can see Dwight Howard has very very low accuracy when it comes to free throws

# Player Style Patterns Excluding Free Throws
(Points - FreeThrows)/FieldGoals
myplot((Points - FreeThrows)/FieldGoals)
# From the graph we can say that in the recent year most players were aiming for 3 pointers.